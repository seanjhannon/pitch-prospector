#!/usr/bin/env python3
"""
ONE-TIME SCRIPT: Migrate pre-processed at-bat level Parquet data to Supabase
This script reads the already-processed at-bat level Parquet files and inserts them into Supabase.
    ...
    try:
        ...
        for _, row in df.iterrows():
            atbat_data = {
                ...
                "pitch_sequence": eval(row["pitch_sequence"]) if isinstance(row["pitch_sequence"], str) else row["pitch_sequence"],
                "pitch_level_data": eval(row["pitch_level_data"]) if isinstance(row["pitch_level_data"], str) else row["pitch_level_data"]
            }
...
def main():
    ...
    print("🚀 Starting pre-processed Parquet to Supabase migration...")
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(process_processed_parquet_file, file_path) for file_path in parquet_files]
